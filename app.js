...
app.get('/:nome/conteudos', async(req, res)=>{
  ...
  const [ posts, rows ] = await mysql.query(`SELECT * FROM posts WHERE nome = ?`, [req.params.nome])
  const [ userMysqlProfile, rowsProfile ] = await mysql.query(`SELECT * FROM users WHERE nome = ?`, [req.params.nome])
  if(user && userProfile){
    res.render('conteudos', {
      posts: posts,
      user: userMysqlProfile,
      message: `
        <div class="dropdown">
          <a href="" class="d-block link-body-emphasis text-decoration-none dropdown-toggle text-muted" data-bs-toggle="dropdown" aria-expanded="false">
            <strong>
